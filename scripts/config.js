...
async function populateForm (node, type, vmid) {
	...
	addFormLine("resources", "Memory", {type: "number", value: config.data.memory / 1024, min: 0}, "GiB");
	...
	if (type === "qemu") {
		...
		while(Object.hasOwn(config.data, `sata${i}`)){
			...
			addFormLine("resources", `SATA ${i}`, {type: "number", value: sizeUnit === "GiB" ? sizeNum.toFixed(3) : (sizeNum / 1024).toFixed(3), min: 0}, "GiB");
			i++;
		}
	}
	else {
		...
		addFormLine("resources", "Root FS", {type: "number", value: sizeUnit === "GiB" ? sizeNum.toFixed(3) : (sizeNum / 1024).toFixed(3), min: 0}, "GiB");
	}
}
